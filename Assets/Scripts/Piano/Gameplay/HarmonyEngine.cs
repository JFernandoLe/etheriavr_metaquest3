...
{
    /// <summary>
    /// Evalúa la calidad de timing de una nota pulsada respecto a la esperada.
    /// Retorna 1.0 si está dentro de la ventana perfecta, decae cuadráticamente
    /// hasta 0.0 al llegar al límite de la ventana de scoring.
    /// </summary>
    ...
    /// <returns>Calidad entre 0.0 (fallido) y 1.0 (perfecto)</returns>
    ...
    /// <summary>
    /// Valida si una nota MIDI tocada coincide con la esperada y está dentro de la ventana de tiempo.
    /// </summary>
    /// <param name="notaEsperada">Nota MIDI esperada por la partitura</param>
    /// <param name="notaTocada">Nota MIDI pulsada por el usuario</param>
    ...
    /// <returns>True si la nota y el timing son correctos</returns>
    public static bool ValidarAcierto(int notaEsperada, int notaTocada, float tiempoDesviacion, float hitWindow)
    {
        return notaEsperada == notaTocada && tiempoDesviacion <= hitWindow;
    }
}
