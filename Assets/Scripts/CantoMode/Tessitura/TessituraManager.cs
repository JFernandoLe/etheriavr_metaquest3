...
public class TessituraManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentNoteText != null) currentNoteText.text = MusicalNoteUtility.MidiToNoteName(midi);
        if (rangeText != null) rangeText.text = $"{MusicalNoteUtility.MidiToNoteName(minMidi)} - {MusicalNoteUtility.MidiToNoteName(maxMidi)}";
    }
